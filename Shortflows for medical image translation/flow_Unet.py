...
class DiffusionModelUNet(nn.Module):
    ...
    def forward(
        ...
        """
        # 1. time
        t_emb = get_timestep_embedding(timesteps_t, self.block_out_channels[0])
        r_emb = get_timestep_embedding(timesteps_r, self.block_out_channels[0])
        ...
        r_emb = r_emb.to(dtype=x.dtype)
        emb_r = self.time_embed_r(r_emb)
